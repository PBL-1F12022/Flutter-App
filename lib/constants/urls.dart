const baseUrl = "https://pbl2022-project-backend.herokuapp.com";
// const baseUrl = "localhost:3000";
const getProjectsUrl = baseUrl + "/projects";
const createProjectUrl = baseUrl + "/projects";

const signUpInvestorUrl = baseUrl + "/investor";
const loginInvestorUrl = baseUrl + '/investor/login';
const getInvestmentsUrl = baseUrl + '/investor/investments';

const signUpEntrepreneurUrl = baseUrl + "/entrepreneur";
const loginEntrepreneurUrl = baseUrl + '/entrepreneur/login';
const getMyProjectsUrl = baseUrl + '/entrepreneur/invested_details';
const getInvestorsListUrl = baseUrl + '/entrepreneur/invested_details';

const coinsUrl = baseUrl + "/coins/";

const bookmarkUrl = baseUrl + '/bookmark';
const investUrl = baseUrl + '/investor/invest';
